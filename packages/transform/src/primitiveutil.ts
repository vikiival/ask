...
export class AbiUtils {
  ...
  static checkActionName(str: string): void {
      ...
      assert.ok(str.length <= 21, `Action Name:${str} should be less than 21 characters.`);
  }
  ...
  static checkDatabaseName(name: string): void {
      ...
      assert.ok(name.length <= 12, `Table name Name:${name} should be less than 12 characters.`);
      for (let aChar of name) {
          assert.ok(AbiUtils.DATABASE_CHARSETS.includes(aChar), `Table name:${name} should only contain the chars:${AbiUtils.DATABASE_CHARSETS}`);
      }
  }
}
export class Verify {
